...
{\large
    \begin{tabu}{c >{\bfseries}c c c c }
	\multicolumn{5}{c}{The ``Males'' (common consonant clusters)}\\
	\rowfont{\small}
    Glyph & \nf{Rom.} & \multicolumn{2}{c}{Pron. (Dem./Bym.)} & Name \\
	\hline 
...
Approximant &          &             &        &          &              & \aph{j}     & \aph{\textturnmrleg} \\
Lateral Appr.&          &           &         & \aph{l}\\
\end{tabular}
...
\begin{tabular}[]{|*{30}{c|}}
    ...
    \d{l} & \d{\l} & m & \d{m} & n & \d{n} & \~n & o & ò & p & r & \d{r} & s & \v{s} & t & \=t & \textipa{W} \\
    
    \apa{\s{l}} & \apa{\s{l}:} & \apa{m} & \apa{\s{m}} & \apa{n}&  \apa{\s{n}} & \apa{\textltailn} & \apa{o} & \apa{O:} & \apa{p} & \apa{r/R} & \apa{\s{r}} & \apa{s} & \apa{S} & \apa{t} & \apa{t:} & \apa{W}  \\
...
(The \textipa{:} symbol on vowels in this chart is a shorthand to mean that the vowel is stressed).
