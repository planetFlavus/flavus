...
The Wanderer is a bright, spectacular sight when in opposition, that is when it is closest to Flavus, and reaches maximum altitude at midnight. In fact, Flavans measure years as spanning between consecutive oppositions (which they call "visits") of the Wanderer, instead of through the motion of their sun on the fixed stars as it is done on Earth. Thus, Flavans prefer the synodic period of the Wanderer with respect to Flavus, rather than Flavus' own sidereal period, to act as a measure of time. This choice is coherent both with the absence of detectable seasons and with the secondary role the "fixed" stars have in Flavan cosmology. One Visit is equal to about $1.32$ Earth years.

\cmmnt{finish finish finish}


\subsection{Bicolourism and Pantheon}

The base belief system on Flavus is a polytheistic system of ``deities'' which are invoked as incarnations, representations and explanation of natural and human phenomena, but curiously are not worshipped nor interacted with. There is no personal relationship with the gods; they are thus simply in the role of supernatural or historical figures which have affected and have been affected by the flow of events and the inner machinery of the world, not really substantially different from humans if not in scale and scope of their lives.

Behind all Flavan philosophy and spirituality is a dualistic humoural belief system which researchers have come to call \textbf{Bicolourism}. This states that two ``colours'', or ``fluids'' are the fundamental elements that constitute the Universe and all it contains, these are:

\vspace{1em}

\begin{minipage}{0.5\textwidth}
    \begin{center}
        {\Large \textbf{The Yellow}} \\
        (\textbf{boryg} \apa{"bO.r1g})
    \end{center}
...
\begin{minipage}{0.5\textwidth}
    \begin{center}
        {\Large \textbf{The Blue}} \\
        (\textbf{gmyrta} \apa{"gm\s{r}.ta})
    \end{center}
...
\cmmnt{continue, add \emph{baba gmyrta} and the other guy}
...
The language that Flavans speak in does not seem to have any discernible relationship with any ever spoken on Earth. It is thought therefore that most of the basic structure of the Flavan language has indipendently developed on the planet. As a result, it includes a combination of alien and unfamiliar constructions and more usual features that might have the character of linguistic universals of all humans. It is hard however to make this kind of generalizing statements with confidence since Flavus has likely only ever hosted not much more than \textbf{one language} at any given time: the constant rearrangement of population, and an army of travelling merchants perennially jumping from village to village, has guaranteed that differences in all aspects of culture, language in particular, are smoothed out and always remain somewhat modest. Thus Flavan enjoys limited regional variations, almost entirely in pronunciation, and remains mutually intelligible from the left-most of Bymarog villages to the antipodal dialect of the Demorog settlements in the far-right.

As per the origins of this language, little is known about Flavans in general before around 1500 years ago. Villages at this time likely only occupied a smaller area in the central region, and the small population used a strongly agglutinative language which is known as \textbf{classical Flavan}, written in a rune-like alphabetic script meant for carving. Classical Flavan's pronunciation is unknown but has been tentatively reconstructed as having the unusual four-vowel inventory \ipa{a e o 1}. It is also important that around this time lies the dating of the latest artefacts attributed to \textbf{Civilization B}, or so-called ``water people'', which disappeared not much time later. Civ B has not been completely deciphered as of today, but from what is known it seems to be a distant relative of classical Flavan, leading to speculations of a \textbf{proto-Flavan} language much earlier in time.

Between about -300Vs and -230Vs (roughly between 900 and 800 years ago) population increases dramatically as technological advances allowed Flavans to explore and colonize the entire survivable Northern cap, resulting in the branching of the Bymarog\footnote{the name \textbf{bymarog} (flame-writer) is used here anacronistically, since the modern script has not been introduced yet at this point.} culture in the somewhat isolated left region. In -183Vs, the Bymarog \textbf{Sun Empire} is founded. The empire quickly expands to the whole Bymarog strip but does not step into Demorog territory; rather migratory flows are blocked and Demorog-Bymarog commerce hindered. Demorog prefer continuing with a village-based social structure.

The cultural (and linguistic) divide grows bigger with time as long as the Sun Empire persists. It's only after the opening of \textbf{the School of Karobet} (by definition in 0Vs), which introduced new writing tools and the \textbf{modern Flavan script} or \textbf{mapaban}, an abugida, that one begins to find an immense amount of written documents, most importantly maps and village registries, which testify that strong changes have already begun in the spoken language, mostly moving it into slightly fusional territory. The court of the Sun Empress opts for the adoption of this new Demorog invention, but gives it own stylistical spin - the flame-like thin strokes are the etimology for the ``flame-writer'' namesake.

In 398Vs, after over seven centuries, the Sun Empire falls apart and Demorog-Bymarog relationships reopen. The two languages merge again and turn into dialects of a single, new language, \textbf{Modern Flavan}. Throughout this millennium-long failed experiment in centralized government, Flavan has evolved significantly in syntax/grammar (developing a very strict word order for example) and phonology (the Classical Flavan phoneme \ipa{1} was unstable and underwent fragmentation into significantly different sounds in the various dialects). 

The present reference grammar aims for an introduction of the basics of this final unified language.
...
\newcommand{\INAL}{\grammar{inal}}
\newcommand{\AL}{\grammar{al}}
...
\begin{itemize}
    ...
    \item \textbf{ytta assimilation:} when not before another vowel, \ipa{1r}, \ipa{1m}, \ipa{1n} become a syllabic consonant \apa{\s{r}}, \apa{\s{m}}, \apa{\s{n}}. Example: \textbf{yrk} \flav{yrk} \ipa{\s{r}k}, \emph{and},  \textbf{kym} \ipa{k\s{m}}, \emph{while}. Some speaker also assimilate \ipa{1N} \textrightarrow \apa{\s{N}}, but this is fairly rare. Assimilation does not happen if the \textbf{y} is preceded by the same consonant that would become syllabic; e.g. \textbf{ryrga} \flav{ryrga} \ipa{"r1rga}, \emph{friendly}, and not \ipa{"r\s{r}ga}.
...
\begin{tabular}[]{|*{30}{c|}}
    \hline
    a & b & c & d & \=d & \dh & e & è & f & g & i & \`\i & \"\i & ȉ & \j & k & x & l \\

    \apa{a} & \apa{b} & c & \apa{d} & \apa{d:} & \apa{D} & \apa{e} & \apa{E:} & \apa{f} & \apa{g} & \apa{i} & \apa{i:} & \apa{1} & \apa{1:} & \apa{j} & \apa{k} & \apa{x} & \apa{l} \\
    
    \hline
    
    \d{l}  & m & \d{m} & n & \d{n} & \~n & o & ò & p & r & \d{r} & \`{\d{r}} & s & \v{s} & t & \=t & \textipa{W} & z \\
    
    \apa{\s{l}}  & \apa{m} & \apa{\s{m}} & \apa{n}&  \apa{\s{n}} & \apa{\textltailn} & \apa{o} & \apa{O:} & \apa{p} & \apa{r/R} & \apa{\s{r}} & \apa{\s{r}:} & \apa{s} & \apa{S} & \apa{t} & \apa{t:} & \apa{W} & \apa{z}  \\
...
\begin{itemize}
    ...
    %\item Copyless focus expulsion can get ambiguous if there isn't sufficient pause between neighbouring sentences (i.e. in written Flavan). In these higher-risk situation expulsion is performed by keeping a pronoun in the original position; e.g., the expulsion of \textbf{dhlorg} in the third example above would go \textbf{Shlem nar rbam dhlarg}.
...
For singular and dual countable, indefiniteness is signaled by the adjectives \textbf{e} and \textbf{ka}, the literal cardinals \emph{one} and \emph{two}, placed after the noun just like normal adjectives. In fact, any cardinal numeral adjective automatically implies indefiniteness: \textbf{bemony pok}, rock.\PL\ three means \emph{three rocks} and not \emph{the three rocks}. In case one needs to emphasize definiteness with a specific number of items that is neither one nor two, the cardinal is moved to before the noun: \textbf{pok bemony}, three rock\PL, \emph{the three rocks}.

In case the number is unknown to some extent (and thus grammatical number is plural), indefiniteness can be marked with adjectives of quantity, such as \textbf{pyka}, \emph{few, small} or \textbf{korba}, \emph{many, large}. Again, to restate definiteness, the adjective is moved to before the noun: \textbf{bemony pyka} \emph{a few rocks}, \textbf{pyka bemony} \emph{the few rocks}.


\cmmnt{add uncountable}
...
\cmmnt{Demonstrative, indefinite}
...
\begin{center}
    ...
    me.\ERG\ man.\ABS\ (eat.\IND.\SIM).(\PPART) see.\IND.\ANT\\
    ...
    \ipa{N\~On "tOreb depa"pat mo"rat:}\\
    me.\ERG\ man.\ABS\ you.\ERG\ book.\ABS\ (give.\IND.\SIM).(\IPART) see.\IND.\ANT\\
...
An example of a relative clause in genitive position:

\begin{center}
    ``The man whose book you read is here.''\\
    \textbf{Tyreb kon ame [my kagmenyr net kon nyfattk]}\\
man.\ERG\ \textbf{kon}.\ERG\ here (is) you.\ERG\ book.\ABS\ him.\GEN.\INAL\ \textbf{kon} read.\IND.\ANT
\end{center}
...
A negation modifier for verbs has already been introduced, \cmmnt{finish}
...
\section{Interrogatives}\label{interrogatives}
\cmmnt{Interrogatives}
...
\subsection*{[Debug] Statistics}

\input{../dictionary/statistics}
